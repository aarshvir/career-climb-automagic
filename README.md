...
export function SeoHead(props: SeoHeadProps) {
  const { title, description, canonicalPath, structuredData } = props;
  ...
  return (
    <Helmet prioritizeSeoTags>
      {/* canonical, robots, OG/Twitter meta, preload hints, and JSON-LD injection */}
    </Helmet>
  );
}
```
...
| --- | --- | --- | --- |
| LCP | Convert hero image to WebP, serve via responsive image component with `fetchpriority="high"` + preload; reduce unused CSS | Engineering | -400ms |
